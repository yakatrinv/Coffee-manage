...
<c:set var="nameCommand" value="discounts" scope="request"/>
<c:set var="sortParam" value="&sortField=id" scope="request"/>
<c:set var="filterParam" value="&searchSum=${requestScope.searchSum}&searchPercent=${requestScope.searchPercent}"
       scope="request"/>
...
<c:set var="discounts" value="${requestScope.pageable.records}" scope="request"/>
...
<form action="coffee-manage">
    ...
    <button type="submit" name="command" value="createDiscount" class="buttonClass">ДОБАВИТЬ СКИДКУ</button>
</form>
<br>

<form action="coffee-manage">
    <label> <input type="number" name="searchSum" placeholder="сумма" value="${requestScope.searchSum}"> </label>
    <label> <input type="number" name="searchPercent" placeholder="процент" value="${requestScope.searchPercent}"> </label>
    <input type="hidden" name="pageNumber" value="${pageNumber}">
    <input type="hidden" name="pageSize" value="${pageSize}">
    <button type="submit" name="command" value="discounts" formmethod="get" class="buttonClass">ПОИСК</button>
</form>
                ...
                <tr>
                    ...
                    <td>
                        <form action="coffee-manage" method="get">
                            ...
                            <button type="submit" name="command" value="editDiscount" class="buttonClass">
                                РЕДАКТИРОВАТЬ
                            </button>
                        </form>
                    </td>
                    <td>
                        <form action="coffee-manage">
                            <c:choose>
                                <c:when test="${discounts.size() eq 1}">
                                    <input type="hidden" name="prevURL"
                            ...
                            <input type="hidden" name="id" value="${discount.id}">
                            <button type="submit" name="command" value="deleteDiscount" class="buttonClass">
                                УДАЛИТЬ
                            </button>
                        </form>
                    </td>
